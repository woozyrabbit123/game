...
def action_confirm_tech_operation(player_inv: PlayerInventory, game_state: any, game_configs: any):
    ...
    if not tech_input_string.replace('.', '', 1).isdigit() or float(tech_input_string) <= 0: set_active_prompt_message("Error: Invalid amount."); tech_input_string = ""; return
    amount = float(tech_input_string); base_heat = game_configs.HEAT_FROM_CRYPTO_TRANSACTION; effective_heat = base_heat 
    ...
    effective_heat = int(round(effective_heat)); current_player_region = game_state_data_cache.current_player_region
    ...
    if tech_transaction_in_progress == "buy_crypto":
        price = game_state.current_crypto_prices.get(coin_for_tech_transaction,0); fee = amount*price*game_configs.TECH_CONTACT_SERVICES['CRYPTO_TRADE']['fee_buy_sell']
        if price == 0: set_active_prompt_message("Error: Price unavailable."); return
        if player_inv.cash >= amount*price+fee: player_inv.cash -= (amount*price+fee); player_inv.add_crypto(coin_for_tech_transaction,amount); 
            if effective_heat > 0: current_player_region.modify_heat(effective_heat); show_event_message_external(f"Bought {amount:.4f} {coin_for_tech_transaction.value}. Heat +{effective_heat} in {region_name_str}.")
        ...
        price = game_state.current_crypto_prices.get(coin_for_tech_transaction,0); fee = amount*price*game_configs.TECH_CONTACT_SERVICES['CRYPTO_TRADE']['fee_buy_sell']
        if price == 0: set_active_prompt_message("Error: Price unavailable."); return
        if player_inv.crypto_wallet.get(coin_for_tech_transaction,0) >= amount: player_inv.remove_crypto(coin_for_tech_transaction,amount); player_inv.cash += (amount*price-fee); 
            if effective_heat > 0: current_player_region.modify_heat(effective_heat); show_event_message_external(f"Sold {amount:.4f} {coin_for_tech_transaction.value}. Heat +{effective_heat} in {region_name_str}.")
        ...
        fee = amount * game_configs.TECH_CONTACT_SERVICES['LAUNDER_CASH']['fee']; launder_heat = int(amount*0.05) 
        if player_inv.cash >= (amount + fee): player_inv.cash -= (amount+fee); 
            player_inv.pending_laundered_sc += amount; player_inv.pending_laundered_sc_arrival_day = game_state.current_day + game_configs.LAUNDERING_DELAY_DAYS
            if launder_heat > 0: current_player_region.modify_heat(launder_heat); show_event_message_external(f"Laundered ${amount:,.2f}. Fee ${fee:,.2f}. Arrives day {player_inv.pending_laundered_sc_arrival_day}. Heat +{launder_heat} in {region_name_str}.")
        ...
        if coin_for_tech_transaction == CryptoCoin.DRUG_COIN and player_inv.crypto_wallet.get(CryptoCoin.DRUG_COIN,0) >= amount: player_inv.remove_crypto(CryptoCoin.DRUG_COIN,amount); player_inv.staked_drug_coin['staked_amount'] += amount; show_event_message_external(f"Staked {amount:.4f} DC.")
        ...
        if coin_for_tech_transaction == CryptoCoin.DRUG_COIN and player_inv.staked_drug_coin['staked_amount'] >= amount:
            player_inv.staked_drug_coin['staked_amount'] -= amount; pending = player_inv.staked_drug_coin['pending_rewards']
        ...
        else: set_active_prompt_message(f"Error: Not enough staked {CryptoCoin.DRUG_COIN.value} or wrong coin."); tech_input_string = ""; return
    current_view = "tech_contact"; tech_input_string = ""; tech_transaction_in_progress = None
    setup_buttons(game_state_data_cache, player_inventory_cache, game_configs_data_cache, game_state_data_cache.current_player_region)
...
def setup_buttons(game_state: any, player_inv: PlayerInventory, game_configs: any, current_region: Region):
    ...
    # ... (rest of setup_buttons for other views, largely unchanged)
        ...
        if current_region and current_region.drug_market_data:
            sorted_drug_names = sorted(current_region.drug_market_data.keys(), key=lambda d: d.value)
            button_y_offset_start = 105 ; line_h = 28 ; current_button_y = button_y_offset_start
            for drug_name_enum in sorted_drug_names:
                drug_data_dict = current_region.drug_market_data[drug_name_enum]; qualities_available = drug_data_dict.get("available_qualities", {})
                ...
                for quality_enum in sorted(qualities_available.keys(), key=lambda q: q.value):
                    ...
                    buy_price = current_region.get_buy_price(drug_name_enum, quality_enum); sell_price = current_region.get_sell_price(drug_name_enum, quality_enum)
                    market_stock = current_region.get_available_stock(drug_name_enum, quality_enum); player_stock_item = player_inv.get_drug_item(drug_name_enum, quality_enum)
                    ...
                    market_buy_sell_buttons.append(Button(buy_btn_x, current_button_y -2, action_button_width, action_button_height, "Buy", functools.partial(action_initiate_buy, drug_name_enum, quality_enum, buy_price, market_stock), is_enabled=can_buy, font=FONT_XSMALL))
                    market_buy_sell_buttons.append(Button(sell_btn_x, current_button_y -2, action_button_width, action_button_height, "Sell", functools.partial(action_initiate_sell, drug_name_enum, quality_enum, sell_price, player_has_stock), is_enabled=can_sell, font=FONT_XSMALL))
        ...
        for i, region_enum in enumerate(RegionName):
            if region_enum == current_region.name: continue
            dest_region_obj = game_state.all_regions[region_enum]; travel_cost = 50 
            btn_text = f"{dest_region_obj.name.value} (${travel_cost})"; can_travel = player_inv.cash >= travel_cost
