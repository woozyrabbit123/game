...
def draw_market_view(
    ...
    region_name_str = (
        ...
        if market_region_data and hasattr(market_region_data.name, "value")
            ...
            str(market_region_data.name)
            if market_region_data and market_region_data.name
            else "Unknown Region"
        )
    )
    draw_text(
    ...
    if not market_region_data or not market_region_data.drug_market_data:
        ...
        for drug_name in sorted_drug_names:
            ...
            for quality_enum in sorted(
                qualities_available.keys(), key=lambda q: q.value
            ):
                ...
                stock = market_region_data.get_available_stock(
                    drug_name, quality_enum, player_inventory_data.heat
                )
                ...
                if "MARKET_ANALYST" in player_inventory_data.unlocked_skills:
                    prev_buy_price = drug_data_dict["available_qualities"][
                        quality_enum
                    ].get("previous_buy_price")
                    prev_sell_price = drug_data_dict["available_qualities"][
                        quality_enum
                    ].get("previous_sell_price")

                    buy_indicator_char, buy_indicator_color = (
                        _get_price_movement_indicator(buy_price, prev_buy_price)
                    )
                    sell_indicator_char, sell_indicator_color = (
                        _get_price_movement_indicator(sell_price, prev_sell_price)
                    )
                    ...
                    draw_text(
                        surface,
                        buy_indicator_char,
                        col_xs["buy"] + buy_price_text_width + indicator_offset_x,
                        y_offset,
                        font=FONT_SMALL,
                        color=buy_indicator_color,
                    )
                    draw_text(
                        surface,
                        sell_indicator_char,
                        col_xs["sell"] + sell_price_text_width + indicator_offset_x,
                        y_offset,
                        font=FONT_SMALL,
                        color=sell_indicator_color,
                    )
                ...
                stock_color = (
                    EMERALD_GREEN
                    if stock > 100
                    else GOLDEN_YELLOW if stock > 10 else IMPERIAL_RED
                )
                draw_text(
                    surface,
                    str(stock),
                ...
                # Trend indicators
                if show_trend_icons:
                    prev_price = drug_data_dict["available_qualities"][
                        quality_enum
                    ].get("previous_sell_price")
...
def draw_transaction_input_view(
    ...
    if drug_for_transaction and quality_for_transaction and input_box_rect:
        drug_name_str = getattr(
            drug_for_transaction, "value", str(drug_for_transaction)
        )
        quality_name_str = quality_for_transaction.name.capitalize()
        draw_text(
            surface,
            f"Item: {drug_name_str} ({quality_name_str})",
            SCREEN_WIDTH // 2,
            info_y,
            font=FONT_MEDIUM,
            color=PLATINUM,
        ...
        draw_text(
            surface,
            f"Price: ${price_for_transaction:.2f}",
            SCREEN_WIDTH // 2,
            info_y,
            font=FONT_SMALL,
            color=TEXT_COLOR,
        ...
        info_y += 25
        max_transact_label = (
            "Your Stock" if current_transaction_type == "sell" else "Market Stock"
        )
        draw_text(
            surface,
            f"{max_transact_label}: {available_for_transaction}",
            SCREEN_WIDTH // 2,
            info_y,
            font=FONT_SMALL,
            color=TEXT_COLOR,
        ...
        draw_input_box(
            ...
            is_active=True,
            cursor_visible=True,
            cursor_pos=len(quantity_input_string),
        )
        info_y += 60
