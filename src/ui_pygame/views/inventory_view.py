...
def draw_inventory_view(
    ...
    if not player_inventory_data.items:
        ...
        for drug_name, qualities in player_inventory_data.items.items():
            for quality, quantity in qualities.items():
                ...
                draw_text(
                    surface,
                    f"{name_str} ({quality_str}): {quantity}",
                    col1_x,
                    y_offset,
                    font=FONT_SMALL,
                    color=TEXT_COLOR,
    ...
    if not player_inventory_data.crypto_wallet:
        ...
        for i, (coin, balance) in enumerate(
            player_inventory_data.crypto_wallet.items()
        ):  # coin is CryptoCoin enum
            ...
            if coin == CryptoCoin.DRUG_COIN:
                ...
                pending = player_inventory_data.staked_drug_coin.get(
                    "pending_rewards", 0
                )
                if staked > 0 or pending > 0:
                    ...
                    y_offset += (
                        line_height * 0.8
                    )  # Adjusted to ensure next item is not overlapping
    ...
    draw_text(
        surface,
        f"Skill Points: {player_inventory_data.skill_points}",
        col3_x + 10,
        y_offset,
        font=FONT_MEDIUM_BOLD,
        color=GOLDEN_YELLOW,
    ...
    if player_inventory_data.unlocked_skills:
        ...
        y_offset += line_height
        for i, skill_id in enumerate(
            player_inventory_data.unlocked_skills
        ):  # skill_id is str
            if i % 2 == 0:
            ...
            draw_text(
                surface,
                f"  - {skill_id.value.replace('_', ' ').title()}",
                col3_x + 20,
                y_offset,
                font=FONT_XSMALL,
                color=LIGHT_GREY,
    ...
    has_upgrades = False
    upgrade_count = 0
    if player_inventory_data.max_capacity > getattr(
        player_inventory_data, "BASE_MAX_CAPACITY", 50
    ):  # Check if capacity upgraded
        ...
        draw_text(
            surface,
            f"  - Expanded Capacity ({player_inventory_data.max_capacity})",
            col3_x + 20,
            y_offset,
            font=FONT_XSMALL,
            color=LIGHT_GREY,
    ...
    if player_inventory_data.ghost_network_access > 0:
        ...
        draw_text(
            surface,
            f"  - Ghost Network ({player_inventory_data.ghost_network_access} days)",
            col3_x + 20,
            y_offset,
            font=FONT_XSMALL,
            color=LIGHT_GREY,
