...
def _draw_tech_shared_info(
    ...
    draw_text(
        surface,
        f"Cash: ${player_inventory_data.cash:,.2f}",
        col1_x,
        current_y_col1,
        font=FONT_MEDIUM,
        color=GOLDEN_YELLOW,
    ...
    if player_inventory_data.crypto_wallet:
        for i, (coin, amount) in enumerate(player_inventory_data.crypto_wallet.items()):
            if amount > 0.00001:  # Only show if substantial amount
                ...
                draw_text(
                    surface,
                    f"  {coin.value}: {amount:.4f} (${value_usd:,.2f})",
                    col1_x + 10,
                    current_y_col1,
                    font=FONT_SMALL,
                    color=NEON_BLUE,
    ...
    if (
        wallet_empty and staked_amount == 0 and pending_rewards == 0
    ):  # If truly nothing in crypto
    ...
    draw_text(
        surface,
        f"Ghost Network Access: {player_inventory_data.ghost_network_access} days",
        col1_x,
        current_y_col1,
        font=FONT_MEDIUM,
        color=TEXT_COLOR,
    ...
    draw_text(
        surface,
        f"Secure Phone: {'Yes' if player_inventory_data.has_secure_phone else 'No'}",
        col1_x,
        current_y_col1,
        font=FONT_MEDIUM,
        color=TEXT_COLOR,
...
def draw_tech_coin_select_view(
    surface: pygame.Surface, tech_buttons: List[Button], ui_state: Dict
):
    ...
    draw_text(
        surface,
        "Choose a cryptocurrency for the transaction:",
        SCREEN_WIDTH // 2,
        145,
        font=FONT_MEDIUM,
        color=PLATINUM,
...
def draw_tech_amount_input_view(
    ...
    coin_name = (
        coin_for_tech_transaction.value.upper()
    ...
    draw_text(
        surface,
        f"Your Cash: ${player_inventory_data.cash:,.2f}",
        SCREEN_WIDTH // 2,
        info_y,
        font=FONT_SMALL,
        color=GOLDEN_YELLOW,
    ...
    if coin_for_tech_transaction:
        ...
        draw_text(
            surface,
            f"Current {coin_name} Price: ${price:,.2f}",
            SCREEN_WIDTH // 2,
            info_y,
            font=FONT_SMALL,
            color=TEXT_COLOR,
    ...
    if "launder" in tech_transaction_in_progress:
        prompt_text = "Enter amount of cash to launder:"
    elif "buy" in tech_transaction_in_progress and coin_for_tech_transaction:
        prompt_text = f"Enter amount of {coin_name} to buy:"
    elif "sell" in tech_transaction_in_progress and coin_for_tech_transaction:
        prompt_text = f"Enter amount of {coin_name} to sell:"
    elif "stake" in tech_transaction_in_progress and coin_for_tech_transaction:
        prompt_text = f"Enter amount of {coin_name} to stake:"
    elif "unstake" in tech_transaction_in_progress and coin_for_tech_transaction:
        prompt_text = f"Enter amount of {coin_name} to unstake:"
    draw_text(
        surface,
        prompt_text,
        SCREEN_WIDTH // 2,
    ...
    if tech_input_box_rect:
        ...
        draw_input_box(
            ...
            is_active=True,
            cursor_visible=True,
            cursor_pos=len(tech_input_string),
        )
        info_y += 60
    ...
    if active_prompt_message and prompt_message_timer > 0:
        prompt_color = (
            IMPERIAL_RED
            if "Error" in active_prompt_message
            or "Invalid" in active_prompt_message
            or "Not enough" in active_prompt_message
            else EMERALD_GREEN
        )
        draw_text(
            surface,
...
def draw_tech_contact_view(
    ...
    if current_tech_view_state == "tech_input_coin_select":
        ...
        draw_tech_amount_input_view(
            surface, player_inventory_data, game_state_data, tech_buttons, ui_state
        )
    else:
        # Clear background with gradient effect
        surface.fill((5, 15, 30))  # Dark blue background
        ...
        if active_prompt_message and prompt_message_timer > 0:
            ...
            msg_color = (
                (40, 20, 20) if "Error" in active_prompt_message else (20, 40, 20)
            )
            pygame.draw.rect(surface, msg_color, msg_rect)
            border_color = (
                IMPERIAL_RED if "Error" in active_prompt_message else EMERALD_GREEN
            )
