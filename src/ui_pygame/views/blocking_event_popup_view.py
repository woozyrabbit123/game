...
from ..ui_theme import (
    ...
    YALE_BLUE, PLATINUM, TEXT_COLOR, OXFORD_BLUE, RICH_BLACK, # Added RICH_BLACK for overlay
    draw_text, draw_panel # Assuming draw_panel is available
)
...
def draw_blocking_event_popup(
    ...
    draw_text(surface, title, popup_rect.centerx, title_y, 
              font=title_font, color=PLATINUM, center_aligned=True)

    # 5. Display messages
    messages = event_data.get('messages', ["No details."])
    message_start_y = title_y + title_font.get_linesize() + 20 # Start messages below title
    ...
    for message_line in messages:
        ...
        current_message_y = draw_text(surface, message_line, popup_x + 30, current_message_y,
                                      font=FONT_SMALL, color=TEXT_COLOR, max_width=max_message_width)
        current_message_y += line_spacing // 2 # Add a bit of space between distinct message entries or paragraphs
