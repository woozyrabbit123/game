...
from .ui_theme import (
    ...
    OXFORD_BLUE, YALE_BLUE, IMPERIAL_RED, PLATINUM, RICH_BLACK, EMERALD_GREEN, GOLDEN_YELLOW,
    draw_text
)
...
def draw_hud(surface: pygame.Surface, player_inventory_data: Optional[any], current_region_data: Optional[any], game_state_data: Optional[any]):
    ...
    for i in range(1, 4):
        ...
        pygame.draw.line(surface, YALE_BLUE, (sep_x, hud_bar_y_start + 10), (sep_x, hud_bar_y_start + hud_bar_height - 10), 1)
    
    # Day display with icon
    if game_state_data and hasattr(game_state_data, 'current_day'): 
        day_x = section_width // 2
        draw_text(surface, "DAY", day_x, hud_text_y_center - 15, font=FONT_XSMALL, color=GOLDEN_YELLOW, center_aligned=True)
        draw_text(surface, f"{game_state_data.current_day}", day_x, hud_text_y_center + 5, font=FONT_MEDIUM, color=PLATINUM, center_aligned=True)
    ...
    if player_inventory_data and hasattr(player_inventory_data, 'cash'):
        ...
        draw_text(surface, "CASH", cash_x, hud_text_y_center - 15, font=FONT_XSMALL, color=EMERALD_GREEN, center_aligned=True)
        cash_text = f"${player_inventory_data.cash:,.0f}"
        draw_text(surface, cash_text, cash_x, hud_text_y_center + 5, font=FONT_MEDIUM, color=EMERALD_GREEN, center_aligned=True)
    ...
    if current_region_data and hasattr(current_region_data, 'name'):
        ...
        draw_text(surface, "LOCATION", region_x, hud_text_y_center - 15, font=FONT_XSMALL, color=GOLDEN_YELLOW, center_aligned=True)
        region_name = current_region_data.name if isinstance(current_region_data.name, str) else current_region_data.name.value
        draw_text(surface, region_name.upper(), region_x, hud_text_y_center + 5, font=FONT_MEDIUM, color=PLATINUM, center_aligned=True)
    
    # Load display with progress bar effect
    if player_inventory_data and hasattr(player_inventory_data, 'current_load') and hasattr(player_inventory_data, 'max_capacity'):
        load_x = (section_width * 3) + (section_width // 2)
        load_percentage = player_inventory_data.current_load / player_inventory_data.max_capacity if player_inventory_data.max_capacity > 0 else 0
        load_color = IMPERIAL_RED if load_percentage > 0.9 else GOLDEN_YELLOW if load_percentage > 0.7 else EMERALD_GREEN
        
        draw_text(surface, "LOAD", load_x, hud_text_y_center - 15, font=FONT_XSMALL, color=load_color, center_aligned=True)
        load_text = f"{player_inventory_data.current_load}/{player_inventory_data.max_capacity}"
        draw_text(surface, load_text, load_x, hud_text_y_center + 5, font=FONT_MEDIUM, color=load_color, center_aligned=True)
    ...
    draw_text(surface, "GAME LOG", log_bg_rect.centerx, header_rect.centery, 
              font=FONT_XSMALL, color=PLATINUM, center_aligned=True)
    ...
    for i, msg in enumerate(ui_log_messages):
        ...
        if i % 2 == 0:
            row_rect = pygame.Rect(log_bg_rect.x + 2, current_log_y - 2, log_bg_rect.width - 4, log_line_height)
            pygame.draw.rect(surface, (*RICH_BLACK, 100), row_rect)
        
        draw_text(surface, msg, log_line_x, current_log_y, 
                  font=FONT_XSMALL, color=PLATINUM, max_width=log_bg_width - 20)
