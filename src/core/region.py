...
class Region:
    ...
    def initialize_drug_market(self, drug_name: Union[DrugName, str], base_buy_price: float,
                              base_sell_price: float, tier: int,
                              initial_stocks: Optional[Dict[DrugQuality, int]] = None):
        drug_name_enum = DrugName(drug_name) if isinstance(drug_name, str) else drug_name
    ...
    def get_buy_price(self, drug_name: DrugName, quality: DrugQuality) -> float:
            ...
            for event in self.active_market_events:
                if event.event_type == EventType.DEMAND_SPIKE and \
                   ...
                   event.target_quality == quality:
                    is_event_driven_price = True; break
            if not is_event_driven_price: return 0.0
        temp_drug = Drug(drug_name.value, drug_data_market["tier"], drug_data_market["base_buy_price"], drug_data_market["base_sell_price"], quality)
        ...
        if not crash_event_applied: # Only apply if no DRUG_MARKET_CRASH took precedence
            for event in self.active_market_events:
                # Check for DEMAND_SPIKE or CHEAP_STASH, as these are typical general events affecting buy price
                if (event.event_type == EventType.DEMAND_SPIKE or event.event_type == EventType.CHEAP_STASH) and \
                   event.target_drug_name == drug_name and \
                   event.target_quality == quality:
    ...
    def get_sell_price(self, drug_name: DrugName, quality: DrugQuality) -> float:
        ...
        if not crash_event_applied:
            for event in self.active_market_events:
                 # Typically DEMAND_SPIKE affects sell price positively for player.
                 # Other events might affect it too, but this is a common one.
                if event.event_type == EventType.DEMAND_SPIKE and \
                   event.target_drug_name == drug_name and \
                   event.target_quality == quality:
        ...
        for event in self.active_market_events:
            if event.event_type == EventType.SUPPLY_DISRUPTION and \
               ...
               hasattr(event, 'min_stock_after_event') and event.min_stock_after_event is not None:
                ...
                # Removed break here, as multiple events could theoretically affect stock,
                # though typically one major event like this would be dominant.
                # If other events also modify stock, they should be allowed to apply.
    ...
    def restock_market(self):
        ...
        for drug_name_enum, drug_data_market in self.drug_market_data.items():
            ...
            for quality_enum_member, drug_data_quality in list(drug_data_market.get("available_qualities", {}).items()):
                ...
                # The line for heat stock reduction factor has been removed as per instructions.
        ...
        # drug_name here is already an enum
        for drug_name_enum, drug_data_market in self.drug_market_data.items():
            for quality_enum_member, drug_data_quality in drug_data_market.get("available_qualities", {}).items():
                ...
                if drug_data_quality["previous_buy_price"] is None:
                    drug_data_quality["previous_buy_price"] = self.get_buy_price(drug_name_enum, quality_enum_member)
                if drug_data_quality["previous_sell_price"] is None:
                    drug_data_quality["previous_sell_price"] = self.get_sell_price(drug_name_enum, quality_enum_member)